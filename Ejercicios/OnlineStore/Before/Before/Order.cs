        ...
        public decimal Total
        {
            get
            {
                ...
                foreach (var item in this.Items)
                {
                    ...
                    if (item.Product.Category == Category.Accessories)
                    {
                    ...
                    if (item.Product.Category == Category.Components)
                    {
                        //itemAmount=itemAmount-discount
                        itemAmount = itemAmount - itemAmount * 5 / 100;
                    }

                    if (item.Product.Category == Category.Bikes)
                    {
                        itemAmount = itemAmount - itemAmount * 15 / 100;
                    }

                    if (item.Product.Category == Category.Cloathing)
                    {
                        decimal discount = 0;
                        if (item.Quantity > 2)
                        {
